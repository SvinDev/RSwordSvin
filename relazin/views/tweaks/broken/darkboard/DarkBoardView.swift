        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                    if errors.isEmpty {
                        alert = DarkBoardAlert(message: "Icons applied. Respring now. After reopening RSwordSvin, initialize SBX again so the post-respring icon fixup can restore the original bundle files.")
                    } else {
